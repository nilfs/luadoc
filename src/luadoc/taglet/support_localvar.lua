-------------------------------------------------------------------------------
-- @release $Id: support_localvar.lua,v 0.01 2012/05/09 23:51 nilfs Exp $
-------------------------------------------------------------------------------
...
local tags = require "luadoc.taglet.support_localvar.tags"
local string = require "string"
local table = require "table"

module 'luadoc.taglet.support_localvar'
...
local function check_local_var (line)
	line = util.trim(line)

	logger:info(line)
	
	local r, _, var_name, value = string.find(line, "%s*local%s*(%w+)%s*=%s([^;]+)")
	if r then
		-- found local value
		logger:debug( util.trim(var_name))
		logger:debug( util.trim(value))
...
local function parse_comment (block, first_line)
	...
	if code ~= nil then
		...
		local local_var = check_local_var(code)
		if local_var then
			block.class = "local_var"
			block.name = local_var.name
			block.param = local_var.value
